...
public sealed class AquiferApiService(HttpClient _aquiferClient) : IReadOnlyAquiferService
{
    public async Task<ResourceContent?> GetResourceContentAsync(int contentId)
    {
        ...
        if (contentId == 366960)
        {
            ...
            var response = JsonSerializer.Deserialize<ResourceContentResponse>(
                json,
        ...
        try
        {
            ...
            var response = await _aquiferClient
                .GetFromJsonAsync<ResourceContentResponse>($"resources/{contentId}/content?contentTextType=html")
